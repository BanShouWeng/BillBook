package com.bsw.billbook.widget.timeselector.view;
...
public class PickerView extends View {
    ...
    static class MyHandler extends Handler {
        ...
        private WeakReference<PickerView> mOuter;

        MyHandler(PickerView pickerView) {
            mOuter = new WeakReference<>(pickerView);
        }

        @Override
        public void handleMessage(Message msg) {
            PickerView outer = mOuter.get();
            if (outer != null) {
                if (Math.abs(outer.mMoveLen) < SPEED) {
                    outer.mMoveLen = 0;
                    if (outer.mTask != null) {
                        outer.mTask.cancel();
                        outer.mTask = null;
                    ...
                    outer.mMoveLen = outer.mMoveLen - outer.mMoveLen / Math.abs(outer.mMoveLen) * SPEED;
                outer.invalidate();
            }
        }
    }
    ...
    private void moveHeadToTail() {
        if (Const.judgeListNull(mDataList) == 0) {
            return;
        }
        if (loop) {
...
//    }
    ...
    private void doUp(@SuppressWarnings("unused") MotionEvent event) {
